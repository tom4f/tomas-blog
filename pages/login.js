  ...
  const getData = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        const resp = res.data;
        console.log(resp.webToken);
        // if error in response

        if (resp.webToken === 'error') {
          setAlert({
        ...
        if (typeof resp.webToken === 'string') {
          ...
          const user = resp.webUser;
          const webToken = resp.webToken;
  ...
  return (
    <div>
      ...
      <article className={loginStyles.container_login}>
        <header className={loginStyles.header_label}>
