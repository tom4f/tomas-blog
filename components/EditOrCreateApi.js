...
const EditOrCreateApi = ({
  ...
  const sendData = async (mode, axiosData) => {
    ...
    const phpApiPath = `${serverPath}/rekreace/api/pdo_${mode}_blog${blogApiScriptExt}.php`;
    // test if new url already exists for create blog
    if (mode === 'create') {
      const { title_url } = axiosData;
      const resp = await fetch(`${apiPath}/api/articles/${title_url}`).catch(
        (err) => console.log(err)
      );
      const respJson = await resp.json().catch((err) => console.log(err));
    ...
    axios
      .post(phpApiPath, axiosData, { timeout: 5000 })
      .then((res) => {
  ...
  const missingStyle = (input) => ({
    background: `${input ? 'green' : 'red'}`,
  });
  ...
  return (
    <>
      ...
      >
        <section
          className={loginStyles.input_section}
          style={{ display: 'flex' }}
        >
          <label>https://olca.cz/</label>
          <input
            style={missingStyle(editArticle.title_url)}
            type='text'
            placeholder={editArticle.title_url}
            onChange={(e) =>
              setEditArticle((prev) => ({ ...prev, title_url: e.target.value }))
            }
        ...
        <section className={loginStyles.input_section}>
          ...
          <input
            style={missingStyle(editArticle.date)}
            type='date'
            placeholder={editArticle.date}
            onChange={(e) =>
              setEditArticle((prev) => ({ ...prev, date: e.target.value }))
            }
        ...
        <section className={loginStyles.input_section}>
          ...
          <input
            style={missingStyle(editArticle.category)}
            type='text'
        ...
        <section className={loginStyles.input_section}>
          ...
          <input
            style={missingStyle(editArticle.title)}
            type='text'
        ...
        <section className={loginStyles.input_section}>
          ...
          <textarea
            style={missingStyle(editArticle.intro)}
            type='text'
            placeholder={editArticle.intro}
            onChange={(e) =>
              setEditArticle((prev) => ({ ...prev, intro: e.target.value }))
            }
        ...
        <section className={loginStyles.input_section}>
          ...
          <input
            style={missingStyle(editArticle.image)}
            type='text'
            placeholder={editArticle.image}
            onChange={(e) =>
              setEditArticle((prev) => ({ ...prev, image: e.target.value }))
            }
        ...
        <section className={loginStyles.input_section}>
          ...
          <textarea
            style={missingStyle(editArticle.body)}
            type='text'
      ...
      {editArticle.id ? (
        <section className={loginStyles.submit_section}>
          <button onClick={() => sendData('delete', axiosDeleteData)}>
            Smazat článek
          </button>
        </section>
      ) : null}
